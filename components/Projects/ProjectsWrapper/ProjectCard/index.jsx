    ...
    <div className='h-full w-full inner-container rounded-xl border flex flex-col md:flex-row py-5   bg-white'>
        ...
        <div className='project-description  relative text-sm md:pr-16 flex flex-col items-center lg:items-start  md:w-2/3 '>
            ...
            <div className='project-links flex space-x-4  md:absolute md:bottom-0 '>
                { githubLink?.length && <Link href={githubLink} target="_blank" className=' cursor-pointer text-black border px-2 py-1 flex items-center space-x-2 rounded'><Image src={github}  class='w-6 h-6' alt='GDSC-img' loader={imageLoader} /> <span>GitHub</span> </Link>}
                { liveLink?.length && <Link href={liveLink} target="_blank" className=' cursor-pointer text-black border px-2 py-1 flex items-center space-x-2 rounded'><Image src={rocket} class='w-6 h-6' alt='GDSC-img' loader={imageLoader} /> <span>Live</span> </Link>}
                { license?.length && <span href={liveLink} target="_blank" className=' cursor-pointer text-black border px-2 py-1 flex items-center space-x-2 rounded'><Image src={licenseLogo} class='w-6 h-6' alt='GDSC-img' loader={imageLoader} /> <span>{license}</span> </span>}
            </div>
        </div>
    </div>
</div>
